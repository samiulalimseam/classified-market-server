...
const Categories = () => {
    ...
    const {data:cats =[]} = useQuery({
        queryKey:['cats'],
        queryFn: ()=> fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/categories`)
        .then(res=>res.json())
    })
    const {data:products =[]} = useQuery({
        queryKey:['products'],
        queryFn: ()=> fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/products`)
        .then(res=>res.json())
    })
    // -------------------------------------------------------
