...
const BuyersTable = () => {
  ...
  const { data: buyers = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/users/${buyerQuery}`)
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/orders/${orderQuery}`)
      const data = await res.json()
      return data;
    }
  })
