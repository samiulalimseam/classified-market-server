...
const SellersTable = () => {
  ...
  const { data: sellers = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/users/${query}`)
  ...
  const { data: products = [] } = useQuery({
    queryKey: ['products', ''],
    queryFn: async () => {
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/products`)
      const data = await res.json()
      return data;
    }
  })
