...
const AddProducts = () => {
    ...
    useEffect(() => {
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/categories`)
            .then(res => res.json())
            .then(data => setCats(data))
            .catch(err => console.error(err))
    }, [])
    useEffect(() => {
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/locations`)
            .then(res => res.json())
            .then(data => setLocations(data))
            .catch(err => console.error(err))
    }, [])
    useEffect(() => {
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/conditions`)
            .then(res => res.json())
    ...
    const handleAddProduct = (data) => {
        ...
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/addproduct`, {
            method: 'POST',
            headers: {
