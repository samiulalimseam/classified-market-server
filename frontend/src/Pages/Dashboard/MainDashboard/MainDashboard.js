...
const MainDashboard = () => {
    ...
    useEffect(() => {
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/products/${accData.uid}`)
            .then(res => res.json())
            .then(data => setProducts(data))
            .catch(err => console.error(err))
    }, [])
    useEffect(() => {
        fetch(`${process.env.NODE_ENV === 'production' ? "" : "http://localhost:5000"}/api/orders/${user.email}`)
            .then(res => res.json())
